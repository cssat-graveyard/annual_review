...
select distinct 
	dbo.IntDate_to_CalDate(af.id_calendar_dim_incident) incident_date
...
from ca_ods.base.rptPlacement rp 
	left join ca_ods.dbo.abuse_fact af
		...
		and datediff(dd, removal_dt, dbo.IntDate_to_CalDate(af.id_calendar_dim_incident)) > 7
		and case 
				when iif(rp.[18bday] < rp.discharge_dt, rp.[18bday], rp.discharge_dt) = '9999-12-31'
				then datediff(dd, rp.removal_dt, datefromparts(@fystop, 06, 30))
...
select
  cd.id_calendar_dim
  ,cd.state_fiscal_yyyy fiscal_yr 
  ,count(rp2.substantiated_allegation_cntr) substantiated_allegations
